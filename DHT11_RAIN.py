...
def send_to_ubidots(temperature, humidity, rain_data):
    payload = {
        TEMPERATURE_LABEL: temperature,
        HUMIDITY_LABEL: humidity,
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"Data sent to Ubidots: Temperature={temperature}, Humidity={humidity}, Rain={rain_data}")
        else:
    ...
    try:
        while True:
            ...
            if temperature is not None and humidity is not None:
                send_to_ubidots(temperature, humidity, rain_data)
            else:
